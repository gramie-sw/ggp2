module TipRepository
  ...
  module ClassMethods
    ...
    def user_ids_with_at_least_result_tips(result:, user_ids:, count:)
      all_by_user_ids(user_ids).all_by_result(result).group_by_user_with_at_least_tips(count).pluck(:user_id)
    end

    def user_ids_with_at_least_missed_tips user_ids:, count:
      all_by_user_ids(user_ids).missed_tips.group_by_user_with_at_least_tips(count).pluck(:user_id)
    end
  end
end