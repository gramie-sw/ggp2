class TipConsecutiveBadge < Badge
  ...
  def eligible_user_ids
    Tip.user_ids_with_at_least_result_tips(result: Tip::RESULTS[result.to_sym], count: count).select do |user_id|
      user_id if has_at_least_consecutive_results(Tip.ordered_results_by_user_id(user_id), Tip::RESULTS[result.to_sym].to_s)
    end
  end
  ...
  def has_at_least_consecutive_results ordered_user_results, result
