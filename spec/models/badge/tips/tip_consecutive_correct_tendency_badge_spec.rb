  ...
  describe '#eligible_user_ids' do
    ...
    it 'returns all user ids which have at least count consecutive correct tendency tips' do
      ...
      expect(TipQueries).to receive(:user_ids_with_at_least_result_tips).with(result: results,
                                                                              user_ids: user_ids, count: subject.achievement) { [2, 3, 4, 5,6] }
      ...
      expect(TipQueries).to receive(:match_position_ordered_results).with(6).and_return(
          [Tip::RESULTS[:correct_tendency_with_one_score], Tip::RESULTS[:correct_tendency_only], nil, Tip::RESULTS[:correct_tendency_with_one_score], nil])
