describe UserStatistic, :type => :model do
  ...
  describe 'correct_tendency_tips_count' do

    it 'should return correct_tendency_tips_count of current_ranking_item' do
      expect(subject.correct_tendency_tips_count).to eq current_ranking_item.correct_tendency_tips_count
    end
  end
  ...
  describe '#correct_tendency_tips_only_ratio' do
    ...
    context 'if matches has been played' do
      ...
      it 'should return rounded ratio' do
        allow(tournament).to receive(:played_match_count).and_return(9)
        allow(subject).to receive(:correct_tendency_tips_count).and_return(4)
